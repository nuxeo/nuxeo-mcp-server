...
@patch("requests.post")
def test_search_repository_probe_401_returns_permission_denied(mock_post: MagicMock) -> None:
...
@patch("requests.post")
def test_search_audit_probe_401_returns_permission_denied(mock_post: MagicMock) -> None:
    mock_post.return_value = _mock_probe(401)
    fn = _register_and_capture(_make_nuxeo())["search_audit"]
